...
export function useFinancialMetrics(
  ...
  return useMemo(() => {
    ...
    // ============================================
    // SOURCE: financial_metrics (n8n v1.7.0+, consolidated in v1.8.0+)
    // ============================================
    ...
    // ============================================
    // ARPU EXTRACTION - SOURCE: financial_metrics (n8n v1.7.0+)
    // Priority: financial_metrics → benchmark → calculated
    // ============================================
    const arpuFromN8n = sanitizeNumericValue(financialMetricsDirect?.arpu_used, null) as number | null;
    const ltvFromN8n = sanitizeNumericValue(financialMetricsDirect?.ltv_used, null) as number | null;
    const cacFromN8n = sanitizeNumericValue(financialMetricsDirect?.cac_used, null) as number | null;
    const churnFromN8n = sanitizeNumericValue(financialMetricsDirect?.monthly_churn_used, null) as number | null;
    const ltvCacTargetFromN8n = sanitizeNumericValue(financialMetricsDirect?.ltv_cac_ratio_target, null) as number | null;
