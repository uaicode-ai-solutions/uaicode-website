...
import { useEveElevenLabs } from "@/hooks/useEveElevenLabs";

interface EveVoiceDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const EveVoiceDialog = ({ open, onOpenChange }: EveVoiceDialogProps) => {
  ...
  }, [open]); // Only trigger on open change
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden glass-card border-amber-500/20 [&>button]:hidden">
        <DialogTitle className="sr-only">Talk to Eve - AI Assistant</DialogTitle>
        ...
        <div className="flex flex-col items-center py-4 border-b border-border/30">
          ...
          <h3 className="mt-2 font-semibold text-foreground">Eve Morgan</h3>
        ...
        <div className="h-[300px] overflow-y-auto p-4 space-y-4">
          ...
          {messages.length === 0 && !isCallActive && !isConnecting && (
            <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
              ...
              <p className="text-sm">Click the button below to start talking with Eve</p>
            </div>
          )}
          ...
          {isConnecting && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
              ...
              <p className="text-sm">Connecting to Eve...</p>
            </div>
          )}
            ...
            >
              ...
              {message.role === "assistant" && (
                <div className="flex-shrink-0 mt-1">
                  <EveAvatar size="sm" isActive={isCallActive} />
                </div>
              )}
          ...
          {/* Speaking indicator - when Eve is speaking */}
          {messages.length > 0 && messages[messages.length - 1].role === "user" && (
            <div className="flex gap-3 justify-start animate-fade-in-up">
              <div className="flex-shrink-0 mt-1">
                <EveAvatar size="sm" isActive />
              </div>
              <div className="bg-gradient-to-br from-secondary via-secondary to-secondary/80 text-foreground rounded-2xl rounded-bl-md px-4 py-3 border border-border/50">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">Eve is speaking</span>
                  <div className="flex items-center gap-1">
        ...
        <div className="p-4 border-t border-border/50 bg-muted/30">
          ...
          <p className="text-center text-xs text-muted-foreground mt-3">
            🎤 Voice powered by AI • Free support
          </p>
        </div>
      </DialogContent>
    </Dialog>
