...
import { useEveChatElevenLabs } from "@/hooks/useEveChatElevenLabs";

interface EveChatDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const EveChatDialog = ({ open, onOpenChange }: EveChatDialogProps) => {
    ...
    if (isCallActive) {
      return (
        <Badge variant="outline" className="mt-2 text-xs border-green-500/50 text-green-500">
          ...
          {isSpeaking ? "Eve is typing..." : "Ready"}
        </Badge>
      );
    }
    return (
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden glass-card border-amber-500/20 [&>button]:hidden">
        <DialogTitle className="sr-only">Chat with Eve - AI Assistant</DialogTitle>
        ...
        <div className="h-[300px] overflow-y-auto p-4 space-y-4">
          ...
          {messages.length === 0 && !isCallActive && !isConnecting && (
            <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
              ...
              <p className="text-sm">Type a message to start chatting with Eve</p>
            </div>
          )}
