...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 font-medium transition-all duration-300 hover:underline hover:scale-[1.02] [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-4 pt-0 animate-fade-in", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
