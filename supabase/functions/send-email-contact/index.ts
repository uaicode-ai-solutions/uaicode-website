...
serve(async (req) => {
  ...
  try {
    ...
    const clientEmailResult = await clientRes.json();
    if (!clientRes.ok) {
      throw new Error(clientEmailResult.message || "Failed to send client email");
    }
    ...
    const teamEmailResult = await teamRes.json();
    if (!teamRes.ok) {
      throw new Error(teamEmailResult.message || "Failed to send team email");
    }
